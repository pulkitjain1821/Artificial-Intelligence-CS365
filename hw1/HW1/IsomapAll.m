function [Y, R, E] = IsomapAll(images, rand, labels, D, n_fcn, n_size,name, options); 
...
if (displ==1)
     ...
     if ~isempty(twod)
         ...
         for j = 1: size(Y.coords{twod} , 2);
            if ( labels(j) == 0)
                plot(Y.coords{twod}(1,j) , Y.coords{twod}(2,j) , 'yd');
            end

            if ( labels(j) == 1)
                plot(Y.coords{twod}(1,j) , Y.coords{twod}(2,j) , 'md');
            end
            if ( labels(j) == 2)
                plot(Y.coords{twod}(1,j) , Y.coords{twod}(2,j) , 'co');
            end

            if ( labels(j) == 3)
                plot(Y.coords{twod}(1,j) , Y.coords{twod}(2,j) , 'gd');
            end
            if ( labels(j) == 4)
                plot(Y.coords{twod}(1,j) , Y.coords{twod}(2,j) , 'rd');
            end
         ...
         hold on
         plot(poses(1,rand),poses(2,rand),'k*');
         hold on
