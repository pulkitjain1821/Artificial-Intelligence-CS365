function [Y, R, E,Path] = Isomap_g(D,data,not_to_show, n_fcn, n_size, options) 
...
%%%%  removing unwanted edges by setting ingfinite %%%%%%
D1=D;
for i = 1:size(not_to_show,2)
    D1(not_to_show(i),:)=INF;
    D1(:,not_to_show(i))=INF;
end

%%%%% shortest path calculation using builtin funciton %%%%%
temp = reshape(D1,size(D1,1)*size(D1,2),1);
mytmp = 1:size(D1,1);
newtmp = [];
for i = 1:size(D1,1)
    newtmp = [newtmp;mytmp];
...
new2tmp = newtmp';
use1 = reshape(newtmp,size(D1,1)*size(D1,1),1);
use1 = use1';
use2 = reshape(new2tmp,size(D1,1)*size(D1,1),1);
use2 = use2';
temp = temp';
DG = sparse(use2,use1,temp);
[dist,Path,pred] = graphshortestpath(DG,1,161);
...
if (displ==1)
     ...
     if ~isempty(twod)
         ...
         %%%% displaying only required points  and path %%%%
         A = 1:size(Y.coords{twod},2);
         B = setdiff(A,not_to_show);
         figure;
         hold on;
         plot(Y.coords{twod}(1,B), Y.coords{twod}(2,B), 'ro'); 
         plot(Y.coords{twod}(1,not_to_show), Y.coords{twod}(2,not_to_show), 'ko'); 
         plot(Y.coords{twod}(1,Path),Y.coords{twod}(2,Path),'-bo');
        
         %% showing initial and final points on the path%%%%   
         random = [1 161];
         ...
         for i = 1:size(random,2)
            ...
            plot(xc,yc,'bo')
            title('Path between 0001.png and 0161.png in 2D');
         end 
        hold off;
        
         figure;
         hold on;
         plot3(Y.coords{3}(1,B), Y.coords{3}(2,B),Y.coords{3}(3,B),'ro'); 
         plot3(Y.coords{3}(1,not_to_show), Y.coords{3}(2,not_to_show),Y.coords{3}(3,not_to_show), 'ko'); 
         plot3(Y.coords{3}(1,Path),Y.coords{3}(2,Path),Y.coords{3}(3,Path),'-bo');
         box on;
         view(45,45); 
         title('Path between 0001.png and 0161.png in 3D');
     end
