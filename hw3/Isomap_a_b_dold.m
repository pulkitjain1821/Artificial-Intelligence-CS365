function [Y, R, E] = Isomap_a_b_d(dataarg,D, n_fcn, n_size, options) 
...
elseif nargin < 4
     options = struct('dims',1:15,'overlay',0,'comp',1,'display',1,'verbose',1); 
end
...
if (displ==1)
     ...
     if ~isempty(twod)
         ...
         poses(1,:)=Y.coords{twod}(1,:);
         poses(2,:)=Y.coords{twod}(2,:);  
         ...
         scale2 = range(poses(2,:))/800;
                
         for i = 1:size(random,2)
            newImg=reshape(dataarg(:,random(i)),100,100,3);
            %{
            for j = 1:10000
                if(rem(j,100)==0)
                     newImg(100,j/100,1)= dataarg(j,random(1,i));
                else
                     newImg(rem(j,100)+1,floor(j/100)+1,1)= dataarg(j,random(1,i));
                end
            end

            for j = 10001:20000
                if(rem(j,100)==0)
                    newImg(100,(j-10000)/100,2)= dataarg(j,random(1,i));
                else
                    newImg(rem(j,100)+1,floor((j-10000)/100)+1,2)= dataarg(j,random(1,i));
                end
            end

            for j = 20001:30000
                if(rem(j,100)==0)
                    newImg(100,(j-20000)/100,3)= dataarg(j,random(1,i));
                else
                    newImg(rem(j,100)+1,floor((j-20000)/100)+1,3)= dataarg(j,random(1,i));
                end
            end
            %}
            ...
            %{
            for var1 = 1:size(newImg,1)
                for var2 = 1:size(newImg,2)
                    if(newImg(var1,var2,1)==0 && newImg(var1,var2,2)==0 && newImg(var1,var2,3)==0)
                        newImg(var1,var2,1)=255;
                        newImg(var1,var2,2)=255;
                        newImg(var1,var2,3)=255;
                    end
                end
            end
            %}
            newImg = imresize(newImg,[30 30]);
            newImg = newImg/255 ;
            xc = poses(1,i);
            yc = poses(2,i);
            imagesc([xc xc-100*2*scale1],[yc yc-100*2*scale2], newImg);
