function [Y, R, E] = Isomap(D,data, n_fcn, n_size, options); 
...
temp = reshape(D,size(D,1)*size(D,2),1);
mytmp = 1:size(D,1);
newtmp = [];
for i = 1:size(D,1)
    newtmp = [newtmp;mytmp];
...
new2tmp = newtmp';
use1 = reshape(newtmp,size(D,1)*size(D,1),1);
use1 = use1';
use2 = reshape(new2tmp,size(D,1)*size(D,1),1);
use2 = use2';
temp = temp';
DG = sparse(use2,use1,temp);
[dist,path,pred] = graphshortestpath(DG,1,500);
path;
dist;
