function [Y, R, E] = Isomapc(D,files, n_fcn, n_size, options,data,srcdir); 
...
if (displ==1)
     ...
     if ~isempty(twod)
        ...
        % files = dir([srcdir '/*.' 'png']);
        
         for q=1:length(files)
             if (rem(q,100)==0)
                 img = imread([srcdir '/' files(q).name]);
%                  img = imresize(img, [100 100]);
                 imagesc([Y.coords{twod}(1,q) Y.coords{twod}(1,q)+11],[Y.coords{twod}(2,q)+14  Y.coords{twod}(2,q)],img);
             end
       
          end
%          for q=1
%              k2=reshape(data(:,q),100,300);
%              k2=k2';
%              if (rem(q,100)==1)
% %                 k=repmat(uint8(255.*k2),[1 1 3]);
%                 imagesc([Y.coords{twod}(1,q) Y.coords{twod}(1,q)+7],[Y.coords{twod}(2,q)+7  Y.coords{twod}(2,q)],[1 81]);
%              end
%          end
%          
         if (overlay == 1)
