function [Y, R, E] = TenIsomap(D,not_to_show, n_fcn, n_size, options); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% remove edges correspondng to intersecting
%%%%%%%%%%%%%%%%%%%%%%%%%%% images by setting d value to infinite
D1=D;
for i = 1:size(not_to_show,2)
    D1(not_to_show(1,i),:)=INF;
    D1(:,not_to_show(1,i))=INF;
end
...
if (displ==1)
     ...
     if ~isempty(twod)
         ...
         B = setdiff(A,not_to_show);                %%only non intersecting nodes have to be plotted
         figure;
         hold on;
         plot(Y.coords{twod}(1,B), Y.coords{twod}(2,B), 'ro'); 
         plot(Y.coords{twod}(1,path),Y.coords{twod}(2,path),'-bo');         % plot the computed path in the embedding
